...
class MockGoogleSheetsService
{
    ...
    public function getStatistics() {
        ...
        return [
            ...
            'highest_level' => count($data) > 0 ? max(array_map(function ($row) {
                return (int) ($row[5] ?? 1); }, $data)) : 0,
            'unique_players' => count(array_unique(array_map(function ($row) {
                return $row[1] ?? 'Anonymous'; }, $data))),
            'active_days' => 1
        ];
    }
    ...
    public function isNewRecord($score) {
        ...
        $maxScore = max(array_map(function ($row) {
            return (int) ($row[4] ?? 0); }, $data));
